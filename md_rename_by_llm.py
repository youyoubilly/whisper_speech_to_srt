#!/usr/bin/env python3
"""
Interactive script to rename markdown files with better names based on content.

Usage:
  python md_rename_by_llm.py path/to/file.md
...
def read_md_text(md_path: Path, max_chars: int = MAX_CHARS) -> str:
...
def suggest_filenames(text: str, date_str: str) -> list[str]:
    ...
    try:
        ...
        prompt = f"""Based on the following document excerpt, suggest 5 different filename options in the format: {date_str}-<topic>
...
{date_str}-contract-agreement-document
{date_str}-legal-document-summary
{date_str}-business-contract-terms
{date_str}-agreement-details-overview
{date_str}-contractual-document-review
"""
...
def interactive_rename(md_path: Path, suggestions: list[str]) -> str:
    ...
    while True:
        ...
        if choice == '0':
            custom_name = input(f"Enter custom filename (without .md extension): ").strip()
            if custom_name:
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python md_rename_by_llm.py path/to/file.md")
        sys.exit(1)
    ...
    date_str = get_file_date(md_path)
    ...
    new_filename = interactive_rename(md_path, suggestions)
